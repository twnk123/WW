...
const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      <header className="hidden min-[1050px]:block fixed top-0 left-0 right-0 z-40 transition-colors duration-300">
        <div className={`max-w-7xl mx-auto px-6 md:px-10 transition-all duration-300 ${isScrolled ? 'pt-4' : 'pt-8'}`}>
          <div className="relative flex justify-center items-center h-16 bg-transparent">
            ...
            <div className="absolute right-0 flex items-center space-x-2">
              <button aria-label="Toggle theme" onClick={toggleInvert} title={invertOn ? 'Disable color invert' : 'Invert colors'} className="rounded-full p-3 bg-black/25 hover:bg-black/35 border border-black/40 shadow-sm transition-colors">
      ...
      <header className={`min-[1050px]:hidden fixed top-4 left-4 right-4 z-[60] transition-all duration-300`}>
        <div className={`p-2 rounded-2xl ${isOpen ? 'bg-bg' : 'bg-bg/80 backdrop-blur-sm'}`}>
            <div className="flex justify-between items-center h-12 px-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button onClick={toggleMenu} aria-label="Toggle menu" className="relative w-10 h-10 flex items-center justify-center">
